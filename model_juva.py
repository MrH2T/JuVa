...
def scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False) -> torch.Tensor:
    ...
    if is_causal:
        assert L == S, "For causal attention, query and key must have the same sequence length"
        causal_mask = torch.ones((L, S), device=query.device).tril(diagonal=1)
...
class Attention(nn.Module):
    ...
    def forward(self, x, rope=None, is_causal=False):
        B, N, C = x.shape
...
class JiTBlock(nn.Module):
    ...
    def forward(self, x, c, rope_s, rope_t):
        ...
        shift_msa_s = shift_msa_s.unsqueeze(1).repeat(1, T, 1).view(B*T, 1, D)
        scale_msa_s = scale_msa_s.unsqueeze(1).repeat(1, T, 1).view(B*T, 1,D)
        gate_msa_s = gate_msa_s.unsqueeze(1).repeat(1, T, 1).view(B*T, 1, D)
        ...
        shift_msa_t = shift_msa_t.unsqueeze(1).repeat(1, L, 1, 1).view(B*L, T, D)
        scale_msa_t = scale_msa_t.unsqueeze(1).repeat(1, L, 1, 1).view(B*L, T, D)
        gate_msa_t = gate_msa_t.unsqueeze(1).repeat(1, L, 1, 1).view(B*L, T, D)

        x_t_norm = modulate(self.norm2(x_t), shift_msa_t, scale_msa_t)
        x_t = x_t + gate_msa_t * self.attn_t(x_t_norm, rope=rope_t, is_causal=True)
        x = x_t.view(B, L, T, D).permute(0, 2, 1, 3) 
        # print("after temp attn", x.shape)

        # 3. MLP
        shift_mlp = shift_mlp.unsqueeze(2)
        scale_mlp = scale_mlp.unsqueeze(2)
        gate_mlp = gate_mlp.unsqueeze(2)
